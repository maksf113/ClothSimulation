...
public:
	...
	void setUniform(const std::string& name, bool i);
...
void printProgramLog(uint32_t prog)
{
	int length = 0;
...
Shader::Shader(const std::string& vertPath, const std::string& fragPath)
{
	...
	GL(uint32_t vs = glCreateShader(GL_VERTEX_SHADER));
	GL(uint32_t fs = glCreateShader(GL_FRAGMENT_SHADER));
	GLint vertCompiled;
	GLint fragCompiled;
	GLint linked;
...
inline void Shader::setUniform(const std::string& name, bool i)
{
	int location = getUniformLocation(name);
	GL(glUniform1i(location, i));
}
...
{
	int location = getUniformLocation(name);
	GL(GL(glUniformMatrix4fv(location, 1, GL_FALSE, glm::value_ptr(mat))););
}

inline void Shader::setUniform(const std::string& name, const glm::mat3& mat)
{
	int location = getUniformLocation(name);
	GL(glUniformMatrix3fv(location, 1, GL_FALSE, glm::value_ptr(mat)));
}

void Shader::bindUniformBlock(const std::string& name, int i)
{
	GL(int location = glGetUniformBlockIndex(m_id, name.c_str()));
	GL(glUniformBlockBinding(m_id, location, i));
}
